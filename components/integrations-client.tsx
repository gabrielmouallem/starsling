...
export default function IntegrationsClient({
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Issues (Active Org)</CardTitle>
          <CardDescription>Fetched from /api/issues</CardDescription>
        </CardHeader>
        <CardContent>
          <IssuesList />
        </CardContent>
...
function IssuesList() {
  ...
  if (issues.length === 0)
    return (
      <div className="flex items-center justify-between">
        <span className="text-sm text-muted-foreground">No issues yet</span>
        <Button
          variant="outline"
          size="sm"
          onClick={() => refetch()}
    ...
    <div className="space-y-2">
      ...
      <ul className="divide-y divide-border rounded-md border border-border">
        {issues.map((i) => (
          ...
          >
            <div>
              <div className="font-medium">{i.title ?? `#${i.number}`}</div>
              <div className="text-muted-foreground">
                {i.repo_full_name ?? "repo"} • {i.state ?? "unknown"}
              </div>
            </div>
            {i.number != null && i.repo_full_name && (
              <a
